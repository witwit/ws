...
export async function getTranslations() {
  ...
  const groupedTranslations: Array<GroupedTranslation> = i18n.locales.map(locale => ({
    locale,
    translations: translations.filter(translation => isMatchingLocaleOrLanguage(translation.locale, locale))
  }));

  const mergedTranslations: Array<Translation> = groupedTranslations.map(({ locale, translations }) => ({
    locale,
    data: translations.reverse().reduce((acc, translation) => Object.assign(acc, translation.data), {} as TranslationMap)
  }));

  const parsedTranslations: Array<ParsedTranslation> = mergedTranslations.map(translation => {
